...
export const createCartController: RequestHandler = async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(201).send(<IServerResponse>({ status: 'success', data: savedCart }))
    } catch (error: any) {
        return res.status(error.status || 500).send(<IServerResponse>({ status: 'failed', errors: { message: error.message || error } }))
    }
}
...
export const updateCartController: RequestHandler = async (req: Request, res: Response) => {
    try {
        ...
        res.status(200).send(<IServerResponse>({ status: 'success', data: updateCart }))
    } catch (error: any) {
        return res.status(error.status || 500).send(<IServerResponse>({ status: 'failed', errors: { message: error.message || error } }))
    }
}
...
export const deleteCartController: RequestHandler = async (req: Request, res: Response) => {
    try {
        ...
        res.status(200).send(<IServerResponse>({ status: 'success', data: { message: 'Cart deleted.' } }))
    } catch (error: any) {
        return res.status(error.status || 500).send(<IServerResponse>({ status: 'failed', errors: { message: error.message || error } }))
    }
}
...
export const getCartController: RequestHandler = async (req: Request, res: Response) => {
    try {
        ...
        res.status(200).send(<IServerResponse>({ status: 'success', data: cart }))
    } catch (error: any) {
        return res.status(error.status || 500).send(<IServerResponse>({ status: 'failed', errors: { message: error.message || error } }))
    }
}
...
export const getAllCartsController: RequestHandler = async (req: Request, res: Response) => {
    try {
        ...
        res.status(200).send(<IServerResponse>({ status: 'success', data: carts }))
    } catch (error: any) {
        return res.status(error.status || 500).send(<IServerResponse>({ status: 'failed', errors: { message: error.message || error } }))
    }
}